...
/**
  ...
  */
abstract class ArchieFunction(val paramNames: List[String] = List(), val baseScope: Option[Scope] = None, val container: Option[ArchieInstance] = None) extends FullArchieInstance(ArchieFunctionType.typeInstance) {
...
class ArchieFunctionAdapter (_paramNames: List[String], val f: (Scope) => Option[ArchieInstance], val closure: Option[Scope], _container: Option[ArchieInstance] = None) extends ArchieFunction(_paramNames, closure, _container) {
