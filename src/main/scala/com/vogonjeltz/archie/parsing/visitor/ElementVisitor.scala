...
class ElementVisitor extends ArchieVisitor[Element]{
  ...
  override def visitWhileElement(ctx: WhileElementContext): WhileLoop = {
    implicit val sc = SourceReference(ctx.start.getLine,ctx.getText)
    val conditional = ctx.element(0).accept(this)
    val block = ctx.element(1).accept(this)
    WhileLoop(conditional, block)
  }
